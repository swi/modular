...
To install the Modular Platform, which includes Mojo, you can use `pip` as
described in the [install guide](/max/packages), but for Mojo developers, we
currently recommend using [`pixi`](https://pixi.sh/latest/).
